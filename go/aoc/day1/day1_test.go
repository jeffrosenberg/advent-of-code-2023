...
func TestExtractCalibrationValuePart1(t *testing.T) {
	tests := []struct {
		input    string
		expected int
		skip     bool
	...
	for _, test := range tests {
		...
		t.Run(test.input, func(t *testing.T) {
			t.Log(test.input)
			got := extractCalibrationValue(test.input, false)
			assert.Equal(t, test.expected, got)
		})
	}
}

func TestExtractCalibrationValuePart2(t *testing.T) {
	tests := []struct {
		input    string
		expected int
		skip     bool
	...
	for _, test := range tests {
		...
		t.Run(test.input, func(t *testing.T) {
			t.Log(test.input)
			got := extractCalibrationValue(test.input, true)
			assert.Equal(t, test.expected, got)
		})
	}
}
